...
import SortList from '/imports/ui/newui_components/Options/teamdynamics/create-breakout-room/sort-user-list/component';
...
import styles from '/imports/ui/newui_components/Options/teamdynamics/styles.scss';
import TeamDynamicsHeading from './TeamDynamicsHeading';
import SelectUserModal from './select-user/SelectUserModal';
import Runs from './runs/component';
...
class TeamDynamicsComponent extends PureComponent {
  constructor(props) {
    super(props);
    ...
    this.handleCreateDuo = this.handleCreateDuo.bind(this);
  ...
  renderSelectUserScreen() {
    ...
    return (
      <SortList
        confirm={() => changeFormLevel({ formFillLevel: 2 })}
        users={users}
        room={roomSelected}
  ...
  renderButtonSetLevel(level, label) {
    ...
    return (
      <Button
        ...
        size="lg"
        label={label}
        onClick={() => changeFormLevel({ formFillLevel: level })}
        key={this.btnLevelId}
      />
    );
  }
  ...
  handleCreateDuo() {
    ...
    fetch("https://teamdynamics.getboarded.com/unique_duo_groups", requestOptions)
      ...
      .then(result => {
        console.log(result);
        this.setState({ formLevel: 2, td_data: result });
      })
  ...
  renderCreateRoom() {
    ...
    return (
      ...
      >
        <TeamDynamicsHeading intl={intl} />
        {formLevel == 1 && users.length > 0 &&
          <>
        ...
        {formLevel == 2 && users.length > 0 && <Runs {...this.state} updateState={this.updateState} changeFormLevel={this.props.changeFormLevel} />}
      </div>
    )
  }
...
TeamDynamicsComponent.propTypes = propTypes;

export default injectIntl(TeamDynamicsComponent);